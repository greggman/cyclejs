...
export type DriverInputs<M extends Main> = {
  [k in (string & keyof ReturnType<M>)]: ReturnType<M>[k] extends Observable<infer T> ? Stream<T> : never;
};
...
export type MainInputs<D extends Drivers> = {
  [k in keyof D]: ReturnType<D[k]> extends Stream<infer T> ? Observable<T> : ReturnType<D[k]>
};
